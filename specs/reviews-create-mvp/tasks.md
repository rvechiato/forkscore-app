...
- [x] Confirmar em `docs/MVP_DOMAIN_MODEL.md` e
...
- [x] Criar o modulo `reviews` no backend com entidade, value objects, ports,
...
- [x] Expor `POST /places/{place_id}/reviews` com autenticacao obrigatoria,
      verificacao de local existente, mapeamento de erros e testes de
      integracao HTTP.

## Trilha 3: fluxo e UI minima do frontend

- [x] Substituir o placeholder protegido de `reviews` por um fluxo minimo de
      criacao que reutilize a selecao de local ja existente e permita enviar a
      avaliacao completa.
- [x] Criar models, repository/client, controller e validacoes locais da
      feature `reviews`, espelhando o contrato do backend para submit.

## Trilha 4: testes e validacoes

- [x] Cobrir o fluxo essencial do frontend com testes da feature `reviews` e
      validar os estados de loading, erro e sucesso do submit.
- [x] Rodar `cd backend && .venv/bin/pytest -q` apos a implementacao backend.
- [x] Rodar `cd frontend && flutter analyze` apos a implementacao frontend.
- [x] Rodar `cd frontend && flutter test` apos a implementacao frontend.
