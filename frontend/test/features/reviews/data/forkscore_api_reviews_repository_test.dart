...
void main() {
  group('ForkScoreApiReviewsRepository', () {
    ...
    test('lista reviews completas do local', () async {
      final client = _FakeHttpClient(
        getHandler: (uri, headers) async {
          expect(
            uri.toString(),
            'https://api.example.com/places/place-1/reviews',
          );
          expect(headers['authorization'], 'Bearer token-123');
          return const HttpResponseData(
            statusCode: 200,
            body:
                '{"place_id":"place-1","reviews":[{"id":"rev-1","user":{"id":"user-1","name":"Rafa"},"recommendation":"recommended","overall_rating":4.4,"created_at":"2026-04-28T12:00:00Z","criteria":[{"code":"taste","rating":5,"comment":"Cafe equilibrado."}]}]}',
          );
        },
      );
      final repository = ForkScoreApiReviewsRepository(
      ...
      final reviews = await repository.listPlaceReviews(
        accessToken: 'token-123',
        placeId: 'place-1',
      );

      expect(reviews, hasLength(1));
      expect(reviews.first.id, 'rev-1');
      expect(reviews.first.author.name, 'Rafa');
    });
  });
}
